The :mod:`subjectclient` Python API
================================
...
First create a client instance with your credentials::
    ...
    >>> subject = client.Client(VERSION, USERNAME, PASSWORD, PROJECT_ID, AUTH_URL)

Here ``VERSION`` can be a string or ``subjectclient.api_versions.APIVersion`` obj.
...
session API::
    ...
    >>> subject = client.Client(VERSION, session=sess)
...
there will be a session kept alive for the duration of the with statement::
    ...
    ...                    PROJECT_ID, AUTH_URL) as subject:
...
by passing a connection_pool=True::
    ...
    >>> from subjectclient import client
    >>> subject = client.Client(VERSION, USERNAME, PASSWORD, PROJECT_ID,
...
Then call methods on its managers::
    ...
    >>> fl = subject.flavors.find(ram=512)
    >>> subject.servers.create("my-server", flavor=fl)
    <Server: my-server>

.. warning:: Direct initialization of ``subjectclient.v2.client.Client`` object
