...
import subjectclient.auth_plugin
from subjectclient import client
from subjectclient import exceptions as exc
import subjectclient.extension
from subjectclient.i18n import _
from subjectclient import utils

DEFAULT_MAJOR_OS_SUBJECT_API_VERSION = "1.0"
# The default behaviour of subject client CLI is that CLI negotiates with server
...
# change the default behaviour of subject client CLI.
DEFAULT_OS_SUBJECT_API_VERSION = '1.0'
DEFAULT_NOVA_ENDPOINT_TYPE = 'publicURL'
DEFAULT_NOVA_SERVICE_TYPE = "subject"

HINT_HELP_MSG = (" [hint: use '--os-subject-api-version' flag to show help "
...
class SecretsHelper(object):
    ...
    def save(self, auth_token, management_url, tenant_id):
        ...
        value = "|".join([str(auth_token),
                          str(management_url),
                          str(tenant_id)])
        keyring.set_password("subjectclient_auth", self._make_key(), value)
    ...
    def management_url(self):
        ...
        management_url = None
        try:
            block = keyring.get_password('subjectclient_auth', self._make_key())
            if block:
...
class NovaClientArgumentParser(argparse.ArgumentParser):
    ...
    def _get_option_tuples(self, option_string):
        ...
        deprecated forms of the same options: "subject boot ... --key KEY"
...
class OpenStackComputeShell(object):
    ...
    def get_base_parser(self, argv):
        ...
        parser.add_argument('--version',
                            action='version',
                            version=subjectclient.__version__)
        ...
        parser.add_argument(
            ...
            help=_('Defaults to subject for most actions.'))
        ...
        parser.add_argument(
            '--os-subject-api-version',
            metavar='<subject-api-ver>',
            default=utils.env('OS_SUBJECT_API_VERSION',
                              default=DEFAULT_OS_SUBJECT_API_VERSION),
            help=_('Accepts X, X.Y (where X is major and Y is minor part) or '
                   '"X.latest", defaults to env[OS_SUBJECT_API_VERSION].'))
        ...
        subjectclient.auth_plugin.load_auth_system_opts(parser)
    ...
    def get_subcommand_parser(self, version, do_help=False, argv=None):
        ...
        actions_module = importutils.import_module(
            "subjectclient.v%s.shell" % version.ver_major)
    ...
    def main(self, argv):
        ...
        subjectclient.auth_plugin.discover_auth_systems()

        if not args.os_subject_api_version:
            api_version = api_versions.get_api_version(
                DEFAULT_MAJOR_OS_SUBJECT_API_VERSION)
        else:
            api_version = api_versions.get_api_version(
                args.os_subject_api_version)
        ...
        if os_auth_system and os_auth_system != "keystone":
            warnings.warn(_(
                'subjectclient auth plugins that are not keystone are deprecated.'
                ' Auth plugins should now be done as plugins to keystoneauth'
                ' and selected with --os-auth-type or OS_AUTH_TYPE'))
            auth_plugin = subjectclient.auth_plugin.load_plugin(os_auth_system)
        else:
            auth_plugin = None
        ...
        if not service_type:
            ...
            # service type specified, we use default subject service type.
        ...
        if not skip_auth:
            if not api_version.is_latest():
                if api_version > api_versions.APIVersion("2.0"):
                    if not api_version.matches(subjectclient.API_MIN_VERSION,
                                               subjectclient.API_MAX_VERSION):
                        raise exc.CommandError(
                            _("The specified version isn't supported by "
                              "client. The valid version range is '%(min)s' "
                              "to '%(max)s'") % {
                                "min": subjectclient.API_MIN_VERSION.get_string(),
                                "max": subjectclient.API_MAX_VERSION.get_string()}
                        )
    ...
    def do_bash_completion(self, _args):
        ...
        subject.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main():
    try:
        ...
        print(_("... terminating subject client"), file=sys.stderr)
        sys.exit(130)
